...
extern bool fusb302_debug;

#define FUSB_LOG(fmt, args...)  do{ if(fusb302_debug) printk("[fusbpd]" fmt, ##args); }while(0)
...
extern uint32_t                   PRSwapTimer;                                    // Timer used to bail out of a PR_Swap from the Type-C side if necessary
...
// Debugging Variables
static uint8_t                    USBPDBuf[PDBUFSIZE];                            // Circular buffer of all USB PD messages transferred
static uint8_t                    USBPDBufStart;                                  // Pointer to the first byte of the first message
static uint8_t                    USBPDBufEnd;                                    // Pointer to the last byte of the last message
...
uint32_t                          SinkRequestMaxVoltage;                          // Maximum voltage that the sink will request
uint32_t                          SinkRequestMaxPower;                            // Maximum power the sink will request (used to calculate current as well)
uint32_t                          SinkRequestOpPower;                             // Operating power the sink will request (used to calculate current as well)
...
static uint8_t                    PolicySubIndex;                                 // Sub index for policy states
...
static uint8_t                    CollisionCounter;                               // Collision counter for the policy engine
static uint8_t                    HardResetCounter;                               // The number of times a hard reset has been generated
static uint8_t                    CapsCounter;                                    // Number of capabilities messages sent
static uint32_t                   PolicyStateTimer;                               // Multi-function timer for the different policy states
static uint32_t                   NoResponseTimer;                                // Policy engine no response timer
...
static uint8_t                    MessageIDCounter;                               // Current Tx message ID counter
static uint8_t                    MessageID;                                      // Last received message ID
static bool                     ProtocolMsgRx;                                  // Flag to indicate if we have received a packet
static bool                     ProtocolMsgTx;
static uint8_t                    ProtocolTxBytes;                                // Number of bytes for the Tx FIFO
static uint8_t                    ProtocolTxBuffer[64];                           // Buffer for FUSB300 Tx FIFO (same size as the 300 tx buffer)
static uint8_t                    ProtocolRxBuffer[64];                           // Buffer for FUSB300 Rx FIFO (same size as the 300 rx buffer)
static uint16_t                   ProtocolTimer;                                  // Multi-function timer for the different protocol states
static uint8_t                    ProtocolCRC[4];
...
void set_policy_subindex(uint8_t index)
{
...
void PolicySourceNegotiateCap(void)
{
    ...
    uint8_t objPosition;                                                          // Get the requested object position
...
bool PolicySendHardReset(PolicyState_t nextState, uint32_t delay)
{
    bool Success = false;
    switch (PolicySubIndex)
    {
...
uint8_t PolicySendCommand(uint8_t Command, PolicyState_t nextState, uint8_t subIndex)
{
...
uint8_t PolicySendCommandNoReset(uint8_t Command, PolicyState_t nextState, uint8_t subIndex)
{
    uint8_t Status = STAT_BUSY;
    switch (PDTxStatus)
    {
...
uint8_t PolicySendData(uint8_t MessageType, uint8_t NumDataObjects, doDataObject_t* DataObjects, PolicyState_t nextState, uint8_t subIndex)
{
    uint8_t Status = STAT_BUSY;
    int i;
    if (ProtocolMsgTx == true )
...
uint8_t PolicySendDataNoReset(uint8_t MessageType, uint8_t NumDataObjects, doDataObject_t* DataObjects, PolicyState_t nextState, uint8_t subIndex)
{
    uint8_t Status = STAT_BUSY;
    int i;
    switch (PDTxStatus)
...
{
    uint8_t header1 = 1;                                                          // Declare and set the message size
...
bool StoreUSBPDMessage(sopMainHeader_t Header, doDataObject_t* DataObject, bool transmitter, uint8_t SOPToken)
{
    int i, j, required;
    uint8_t header1;
...
uint8_t GetNextUSBPDMessageSize(void)
{
    uint8_t numBytes;
...
uint8_t GetUSBPDBufferNumBytes(void)
{
    uint8_t bytes;
...
uint8_t GetUSBPDStatusOverview(void)
{
...
uint8_t ReadUSBPDBuffer(unsigned char* pData, unsigned char bytesAvail)
{
...
void WriteSinkRequestSettings(unsigned char* abytData)
{
    ...
    SinkRequestMaxVoltage = (uint32_t) *abytData++;
    SinkRequestMaxVoltage |= ((uint32_t) (*abytData++) << 8);                     // Voltage resolution is 50mV
    uintPower = (uint32_t) *abytData++;
    uintPower |= ((uint32_t) (*abytData++) << 8);
    uintPower |= ((uint32_t) (*abytData++) << 16);
    uintPower |= ((uint32_t) (*abytData++) << 24);
...
void ReadSinkRequestSettings(unsigned char* abytData)
{
    ...
    *abytData++ = (uint8_t) (SinkRequestMaxVoltage & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestMaxVoltage >> 8) & 0xFF);
    *abytData++ = (uint8_t) (SinkRequestOpPower & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestOpPower >> 8) & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestOpPower >> 16) & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestOpPower >> 24) & 0xFF);
    *abytData++ = (uint8_t) (SinkRequestMaxPower & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestMaxPower >> 8) & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestMaxPower >> 16) & 0xFF);
    *abytData++ = (uint8_t) ((SinkRequestMaxPower >> 24) & 0xFF);
}
...
void sendVdmMessage(VdmManager* vdmm, SopType sop, uint32_t* arr, unsigned int length) {
    unsigned int i, j;
