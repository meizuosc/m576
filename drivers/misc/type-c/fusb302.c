...
bool fusb302_debug = false;

#define FUSB_LOG(fmt, args...)  do{ if(fusb302_debug) pr_info("[fusb302]" fmt, ##args); }while(0)

extern struct blocking_notifier_head muic_notifier_list;
...
/* check cable status on boot */
void check_cable_status_fusb302(void)
{
	pr_info("fusb302 check status %d\n", ConnState);

	if (ConnState == AttachedSource)
		blocking_notifier_call_chain(&muic_notifier_list, USB_OTG_ATTACH, NULL);
#if 0
	if (ConnState == AttachedSink)
		blocking_notifier_call_chain(&muic_notifier_list, USB_HOST_ATTACH, NULL);
#endif
}
...
void SetStateUnattached(void)
{
    ...
    pr_info("%s: XXX\n", __func__);
    if (otg_attached)
	blocking_notifier_call_chain(&muic_notifier_list, USB_OTG_DETACH, NULL);
    otg_attached = 0;

#if 0
    blocking_notifier_call_chain(&muic_notifier_list, USB_HOST_DETACH, NULL);
#endif
...
int fusb302_state_kthread(void *x)
{
	...
	INIT_COMPLETION(state_notifier);
...
static int fusb302_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    if (np) {
	fusb->gpio_pd_on = of_get_named_gpio(np, "pd-on", 0);
	if (gpio_request(fusb->gpio_pd_on, "typec_pd"))
		pr_err("request typec pd gpio error\n");
	gpio_direction_output(fusb->gpio_pd_on, 0);

    	fusb->gpio_int_ccd = of_get_named_gpio(np, "int-ccd", 0);
	if (gpio_request(fusb->gpio_int_ccd, "int_typec"))
		pr_err("request int typec gpio error\n");
	fusb->irq_ccd = gpio_to_irq(fusb->gpio_int_ccd);
...
static struct of_device_id fusb302_dt_id[] = {
	{ .compatible = FUSB302_I2C_NAME },
	{ }
};
...
static struct i2c_driver fusb302_i2c_driver = {
        .driver = {
                ...
                .owner = THIS_MODULE,
		.pm = &fusb302_pm_ops,
		.of_match_table = of_match_ptr(fusb302_dt_id),
        },
        .id_table       = fusb302_id,
        .probe          = fusb302_probe,
...
module_i2c_driver(fusb302_i2c_driver);
