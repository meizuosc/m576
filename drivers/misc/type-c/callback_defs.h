...
typedef ModesInfo (*RequestModesInfo)(uint16_t);

// ask DPM if we can enter a mode
typedef bool (*ModeEntryRequest)(uint16_t svid, unsigned int mode_index);

// ask DPM if we can exit a mode - TODO: use mode_index = -1 to signify exit ALL modes?
typedef bool (*ModeExitRequest)(uint16_t svid, unsigned int mode_index);
...
typedef bool (*EnterModeResult)(bool success, uint16_t svid, unsigned int mode_index);

// may need a more complex return code instead of boolean success, if it's important to distinguish
// NAK vs BUSY vs timeout
// TODO: How to manage transition to hard reset if exit mode fails?
typedef void (*ExitModeResult)(bool success, uint16_t svid, unsigned int mode_index);

typedef void (*InformIdentity)(bool success, SopType sop, Identity id);

typedef void (*InformSvids)(bool success, SopType sop, unsigned int num_svids, uint16_t svids[MAX_NUM_SVIDS]);
