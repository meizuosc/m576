...
#ifdef CONFIG_DTS_INV_MPU_IIO
	result = inv_parse_irq(&client->dev, &client->irq);
#endif
...
msleep(100);
	...
	*(struct mpu_platform_data *)dev_get_platdata(&client->dev);
	...
	/* shengliang moved 3 lines here.
	 * Because inv_mpu_configure_ring will request_irq,
	 * the irq handler will need the spinlock.
	 */
