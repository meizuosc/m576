...
#define ADB_CLUSTER0_QOS_FREQ		(1500000)
#define ADB_CLUSTER1_QOS_NUM		(1)
#define ADB_MEMORY_QOS_FREQ		( 828000)
#define ADB_CLUSTER1_QOS_FREQ		(1600000)
#define ADB_DEVICE_QOS_FREQ		( 400000)

static struct pm_qos_request adb_cluster1_qos;
static struct pm_qos_request adb_cluster0_qos;
static struct pm_qos_request adb_cluster1_num_qos;
static struct pm_qos_request adb_bus_qos;
static struct pm_qos_request adb_device_qos;
				...
				size_t count, loff_t *pos)
{
	...
	if (adb_pm_qos_enable && count > PM_QOS_REQUEST_SIZE) {
		pm_qos_update_request_timeout(&adb_cluster0_qos, ADB_CLUSTER0_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_cluster1_num_qos, ADB_CLUSTER1_QOS_NUM, 2000000);
		if (cpu_online(NR_CLUST1_CPUS))
			pm_qos_update_request_timeout(&adb_cluster1_qos, ADB_CLUSTER1_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_bus_qos, ADB_MEMORY_QOS_FREQ, 2000000);
		pm_qos_update_request_timeout(&adb_device_qos, ADB_DEVICE_QOS_FREQ, 2000000);
	}
...
static void adb_cleanup(void)
{
	...
	pm_qos_remove_request(&adb_device_qos);
	pm_qos_remove_request(&adb_bus_qos);
	pm_qos_remove_request(&adb_cluster1_qos);
	pm_qos_remove_request(&adb_cluster0_qos);
	pm_qos_remove_request(&adb_cluster1_num_qos);
