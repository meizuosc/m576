...
static int
samsung_usb2phy_dummy_lpa_event(struct notifier_block *nb,
				unsigned long event,
				void *data)
{
	...
	case LPA_PREPARE:
	case LPC_PREPARE:
		if (samsung_usb2phy_dummy_is_active(&sphy->phy))
			err = -EBUSY;
		break;
	default:
		return NOTIFY_DONE;
...
static int samsung_usb2phy_dummy_probe(struct platform_device *pdev)
{
	...
	sphy->lpa_nb.notifier_call = samsung_usb2phy_dummy_lpa_event;
