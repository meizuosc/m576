...
static checksum_block checksum[] =
{
	...
	{0x5500D000+0x9da00000, 0x5500D0C0+0x9da00000, 0x0000000fb98c1f80},
	{0x5500D0C0+0x9da00000, 0x5500D100+0x9da00000, 0x0000000307f80000},
	{0x5500D100+0x9da00000, 0x5500D1C0+0x9da00000, 0x00000005f803a800},
	{0x5500D1C0+0x9da00000, 0x5500D200+0x9da00000, 0x000000040e800000},
	{0x5500D200+0x9da00000, 0x5500D2C0+0x9da00000, 0x0000000e2f5d87f0},
	{0x5500D2C0+0x9da00000, 0x5500D300+0x9da00000, 0x000000060f660000},
	{0x5500D300+0x9da00000, 0x5500D3C0+0x9da00000, 0x0000000a806ce0a8},
	{0x5500D3C0+0x9da00000, 0x5500D400+0x9da00000, 0x0000000411b70000},
	{0x5500D400+0x9da00000, 0x5500D4C0+0x9da00000, 0x0000000c82b7ef20},
	{0x5500D4C0+0x9da00000, 0x5500D500+0x9da00000, 0x000000050f080000},
	{0x5500D500+0x9da00000, 0x5500D5C0+0x9da00000, 0x0000000e7376e360},
	{0x5500D5C0+0x9da00000, 0x5500D600+0x9da00000, 0x000000070b3c0000},
	{0x5500D600+0x9da00000, 0x5500D6C0+0x9da00000, 0x0000000c756de980},
	{0x5500D6C0+0x9da00000, 0x5500D700+0x9da00000, 0x000000060ef80000},
	{0x5500D700+0x9da00000, 0x5500D7C0+0x9da00000, 0x0000000e34768300},
	{0x5500D7C0+0x9da00000, 0x5500D800+0x9da00000, 0x000000050a040000},
	{0x5500D800+0x9da00000, 0x5500D8C0+0x9da00000, 0x0000000e514abb00},
	{0x5500D8C0+0x9da00000, 0x5500D900+0x9da00000, 0x0000000509f00000},
	{0x5500D900+0x9da00000, 0x5500DA20+0x9da00000, 0x000000130f0d9800},
	{0x5500DA40+0x9da00000, 0x5500DAA0+0x9da00000, 0x000000050d140000},
	{0x55019600+0x9da00000, 0x5501A400+0x9da00000, 0x00000051e0e94d1f},
	{0x5502C000+0x9da00000, 0x5502C440+0x9da00000, 0x0000000000010247},
	{0x5504F000+0x9da00000, 0x5504F600+0x9da00000, 0x00000003b1ba4ecd},
	{0x5504F600+0x9da00000, 0x5504FC00+0x9da00000, 0x00000003b1ba2c0e},
	{0x55019000+0x9da00000, 0x55019600+0x9da00000, 0x00000003b17c8f8e},
	{0x5502C440+0x9da00000, 0x5502C880+0x9da00000, 0x00000037185b6ef3},
	{0x5502D000+0x9da00000, 0x5503D400+0x9da00000, 0x0000004582be5707},
};

bool test_checksum(unsigned int *start, unsigned int *end, unsigned long golden_sum)
{
	unsigned int result;
	...
	{
		printk("start 0x%p, end 0x%p %ld != %ld\n", &start[i], end, checksum, golden_sum);
		return MALI_FALSE;
	}
...
bool test_checksum_all(void)
{
	bool is_ok = MALI_TRUE;
	int i;

	for(i = 0 ; i < sizeof(checksum)/sizeof(checksum[0]); i++)
		is_ok &= test_checksum( (unsigned int*)phys_to_virt(checksum[i].start), (unsigned int*)phys_to_virt(checksum[i].end), (unsigned long)checksum[i].golden_sum );

	return is_ok;
}

/* MALI_SEC */
void preload_warm_up(struct kbase_device *kbdev)
{
	unsigned char *stream_reg;
	dma_addr_t  phys_addr;

	stream_reg = (unsigned char *)phys_to_virt(0xF2A00000);

	phys_addr = dma_map_single(kbdev->dev, stream_reg, streamout_size, DMA_BIDIRECTIONAL);
	memcpy(stream_reg, streamout, streamout_size);
	dma_unmap_single(kbdev->dev, phys_addr, streamout_size, DMA_BIDIRECTIONAL);
}

spinlock_t power_change_lock;

bool warm_up(struct kbase_device *kbdev, mali_bool boot)
{
	unsigned long flags;
	unsigned int temp;
	int count = 0, j;
	...
	if(boot == MALI_TRUE)
		spin_lock_init(&power_change_lock);

	stream_reg = (unsigned char *)phys_to_virt(0xF2A00000);

	phys_addr = dma_map_single(kbdev->dev, stream_reg, streamout_size, DMA_BIDIRECTIONAL);
	memset(stream_reg, 0x0, streamout_size);
	dma_unmap_single(kbdev->dev, phys_addr, streamout_size, DMA_BIDIRECTIONAL);

	spin_lock_irqsave(&power_change_lock, flags);

	GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "power initialized\n");

	/* Mali Soft reset */
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_CLEAR      ), 0x00030781);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_MASK       ), 0x00000100);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND        ), 0x00000001);

	do
	{
		temp = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(GPU_IRQ_RAWSTAT));
		if((temp & 0x100) == 0x100)
			break;
		if(count > 5)
		{
			printk("mali_wait_reg timeout %d\n", __LINE__);
			break;
		}
		count++;
		udelay(10);
	...
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND        ), 0x00000008);
	...
	kbase_os_reg_write(kbdev, MMU_REG(MMU_IRQ_MASK               ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(SHADER_PWRON_LO    ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(SHADER_PWRON_HI    ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWRON_LO     ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWRON_HI     ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(L2_PWRON_LO        ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(L2_PWRON_HI        ), 0xFFFFFFFF);

	do {
		temp = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(SHADER_READY_LO));
		if((temp & 0xff) == 0xff)
			break;
		if(count > 5)
		{
			spin_unlock_irqrestore(&power_change_lock, flags);
			return MALI_FALSE;
		}
		udelay(10);
		count++;
	...
	count = 0;
	do {
		temp = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(TILER_READY_LO));
		if(temp & 0x1)
			break;
		if(count > 5)
		{
	...
	count = 0;
	do {
		temp = kbase_os_reg_read(kbdev, GPU_CONTROL_REG(L2_READY_LO));
		if(temp & 0x1)
			break;
		if(count > 5)
		{
	...
	preload_warm_up(kbdev);

	kbase_os_reg_write(kbdev, MMU_AS_REG(0, AS_MEMATTR_HI         ), 0x88888888);
	kbase_os_reg_write(kbdev, MMU_AS_REG(0, AS_MEMATTR_LO         ), 0x88888888);
	kbase_os_reg_write(kbdev, MMU_AS_REG(0, AS_TRANSTAB_LO        ), 0xF2A00007);
	kbase_os_reg_write(kbdev, MMU_AS_REG(0, AS_TRANSTAB_HI        ), 0x00000000);
	kbase_os_reg_write(kbdev, MMU_AS_REG(0, AS_COMMAND            ), 0x00000001);

	kbase_os_reg_write(kbdev, MMU_AS_REG(1, AS_MEMATTR_HI         ), 0x88888888);
	kbase_os_reg_write(kbdev, MMU_AS_REG(1, AS_MEMATTR_LO         ), 0x88888888);
	kbase_os_reg_write(kbdev, MMU_AS_REG(1, AS_TRANSTAB_LO        ), 0xF2A00007);
	kbase_os_reg_write(kbdev, MMU_AS_REG(1, AS_TRANSTAB_HI        ), 0x00000000);
	kbase_os_reg_write(kbdev, MMU_AS_REG(1, AS_COMMAND            ), 0x00000001);

	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_CLEAR       ), 0x00000081);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_MASK        ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_CLEAR       ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_MASK        ), 0x00000000);

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_LO     ), 0xDE206200);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_CONFIG_NEXT      ), 0x00083701);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_COMMAND_NEXT     ), 0x00000001);

	udelay(360);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_CLEAR       ), 0x00000002);

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_LO     ), 0xE95B53C0);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_CONFIG_NEXT      ), 0x00083F00);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_COMMAND_NEXT     ), 0x00000001);

	udelay(125);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_CLEAR       ), 0x00000001);

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_LO     ), 0xE95B5340);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_CONFIG_NEXT      ), 0x00083F01);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_COMMAND_NEXT     ), 0x00000001);

	udelay(125);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_CLEAR       ), 0x00000002);

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_LO     ), 0xE9512380);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_CONFIG_NEXT      ), 0x00083701);
	...
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_LO     ), 0xDE438380);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_CONFIG_NEXT      ), 0x00083F01);
	...
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_LO     ), 0xE9512400);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_CONFIG_NEXT      ), 0x00083700);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_COMMAND_NEXT     ), 0x00000001);

	udelay(170);
	kbase_os_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_CLEAR       ), 0x00000001);

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_LO     ), 0xDE438400);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_CONFIG_NEXT      ), 0x00083F00);
	...
	for(j = 0 ; j < 8 ; j++)
	{
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_LO     ), 0xDE204580);
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_HEAD_NEXT_HI     ), 0x00000000);
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_LO ), (0x1 << j));
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_AFFINITY_NEXT_HI ), 0x00000000);
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_CONFIG_NEXT      ), 0x00083F01);
		kbase_os_reg_write(kbdev, JOB_SLOT_REG(1, JS_COMMAND_NEXT     ), 0x00000001);
		...
		stream_reg32 = (u32 *)phys_to_virt(0x5500e580 + 0xa8de0000);
		phys_addr = dma_map_single(kbdev->dev, stream_reg32, 4, DMA_BIDIRECTIONAL);
		memset(stream_reg32, 0x0, 4);
		dma_unmap_single(kbdev->dev, phys_addr, 4, DMA_BIDIRECTIONAL);
	}

	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_LO     ), 0xDE206280);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_HEAD_NEXT_HI     ), 0x00000000);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_LO ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_AFFINITY_NEXT_HI ), 0xFFFFFFFF);
	kbase_os_reg_write(kbdev, JOB_SLOT_REG(0, JS_CONFIG_NEXT      ), 0x00083700);
	...
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND         ), 0x00000006);

	udelay(100);

	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND         ), 0x00000004);

	udelay(100);

	if(test_checksum_all() == MALI_FALSE)
	{
	...
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_CLEAR      ), 0x00030781);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_MASK       ), 0x00000100);
	kbase_os_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND        ), 0x00000002);
...
void kbase_pm_clock_on(struct kbase_device *kbdev, mali_bool is_resume)
{
	...
	if (reset_required) {
		...
		{
			first_call = 0;
			warm_up(kbdev, MALI_TRUE);
			for(i = 0; i < 4; i++)
				warm_up(kbdev, MALI_FALSE);
		}
		else {
			do {
				if(warm_up(kbdev, MALI_FALSE) == MALI_TRUE)
					break;
				if(count == 1)
					break;
				count++;
...
#if SLSI_INTEGRATION
	if (kbdev->pm.callback_power_dvfs_on)
		kbdev->pm.callback_power_dvfs_on(kbdev);
#endif
