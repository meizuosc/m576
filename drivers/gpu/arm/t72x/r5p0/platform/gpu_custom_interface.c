...
static ssize_t show_reset_count(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "%d", platform->reset_count);
...
static ssize_t show_data_invalid_fault_count(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "%d", platform->data_invalid_fault_count);
...
static ssize_t show_mmu_fault_count(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "%d", platform->mmu_fault_count);
...
#endif
DEVICE_ATTR(reset_count, S_IRUGO, show_reset_count, NULL);
DEVICE_ATTR(data_invalid_fault_count, S_IRUGO, show_data_invalid_fault_count, NULL);
DEVICE_ATTR(mmu_fault_count, S_IRUGO, show_mmu_fault_count, NULL);
...
int gpu_create_sysfs_file(struct device *dev)
{
	...
	if (device_create_file(dev, &dev_attr_reset_count)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [reset_count]\n");
		goto out;
	}

	if (device_create_file(dev, &dev_attr_data_invalid_fault_count)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [data_invalid_fault_count]\n");
		goto out;
	}

	if (device_create_file(dev, &dev_attr_mmu_fault_count)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [mmu_fault_count]\n");
		goto out;
	}
...
#endif
	device_remove_file(dev, &dev_attr_reset_count);
	device_remove_file(dev, &dev_attr_data_invalid_fault_count);
	device_remove_file(dev, &dev_attr_mmu_fault_count);
}
