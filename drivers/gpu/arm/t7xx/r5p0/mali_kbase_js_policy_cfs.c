...
enum dequeue_result {
	KBASEP_JS_NO_JOB,
	KBASEP_JS_INCORRECT_MODE,
	KBASEP_JS_JOB_DEQUEUED
};
...
void secure_mode_enable(struct kbase_device *kbdev)
{
	if (kbdev->js_data.secure_ops != NULL) {
		int res;

		/* Make sure caches are flushed, so
		 * that we don't get any late evictions
		 * to normal memory after we have switched
		 * to secure mode.
		 */
		kbasep_js_cacheclean(kbdev);

		/* Switch GPU to secure mode */
		res = kbdev->js_data.secure_ops->secure_mode_enable();
		/* MALI_SEC_SECURE_RENDERING */
		BUG_ON(res != SMC_CALL_SUCCESS);

		kbdev->js_data.runpool_irq.secure_mode = MALI_TRUE;
	}
}

void secure_mode_disable(struct kbase_device *kbdev)
{
	if (kbdev->js_data.secure_ops != NULL) {
		int res;

		/* Temporary cache clean, should
		 * be replaced with a GPU reset in
		 * secure world.
		 */
		kbasep_js_cacheclean(kbdev);

		/* Switch GPU to non-secure mode */
		res = kbdev->js_data.secure_ops->secure_mode_disable();
		/* MALI_SEC_SECURE_RENDERING */
		BUG_ON(res != SMC_CALL_SUCCESS);

		kbdev->js_data.runpool_irq.secure_mode = MALI_FALSE;
	}
}

STATIC enum dequeue_result dequeue_job(struct kbase_device *kbdev,
	...
	if (kbasep_js_is_submit_allowed(js_devdata, kctx) != MALI_FALSE) {
		...
		while (variants_supported != 0) {
			...
			if (!list_empty(job_list)) {
				...
				front_atom = list_entry(job_list->next, struct kbase_jd_atom, sched_info.cfs.list);

				/* Check security state */
				if (kbdev->js_data.runpool_irq.secure_mode != front_atom->secure_mode) {
					/* Not in correct mode, take action */
					if (kbasep_jm_nr_jobs_submitted_total(kbdev) > 0) {
						/*
						 * We are not in the correct GPU mode for this job, and we
						 * can't switch now because there are jobs already running.
						 */
						kbdev->js_data.runpool_irq.secure_mode_resched = MALI_TRUE;
						return KBASEP_JS_INCORRECT_MODE;
					}

					/* No jobs running, so we can switch GPU mode right now */
					if (front_atom->secure_mode == MALI_TRUE) {
						secure_mode_enable(kbdev);
					} else {
				       ...
				       struct kbase_jd_atom ** const katom_ptr)
{
	...
	list_for_each(pos, &policy_info->scheduled_ctxs_head) {
		...
		if (kctx->jctx.sched_info.runpool.policy_ctx.cfs.process_rt_policy) {
			...
			res = dequeue_job(kbdev, kctx, variants_supported, katom_ptr, job_slot_idx);
			if (res == KBASEP_JS_JOB_DEQUEUED) {
				/* Realtime policy job matched */
				return MALI_TRUE;
			} else 	if (res == KBASEP_JS_INCORRECT_MODE) {
				/* Realtime policy job matched, but GPU is not in correct security state */
				return MALI_FALSE;
			}
		}
	}
	...
	list_for_each(pos, &policy_info->scheduled_ctxs_head) {
		...
		if (kctx->jctx.sched_info.runpool.policy_ctx.cfs.process_rt_policy == MALI_FALSE) {
			...
			/* else if (res == KBASEP_JS_INCORRECT_MODE) => fall-through */
		}
	}
