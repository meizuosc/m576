...
/* MALI_SEC_INTEGRATION */
		...
		exynos_gpu_init_hw(kbdev);
#endif
		kbase_pm_init_hw(kbdev, MALI_TRUE, MALI_FALSE);
...
mali_error kbase_pm_init_hw(struct kbase_device *kbdev, mali_bool enable_irqs, mali_bool enabledebug)
{
	...
	/* MALI_SEC_INTEGRATION */
	if (kbdev->pm.callback_power_change_dvfs_level)
		kbdev->pm.callback_power_change_dvfs_level(kbdev, enabledebug);
