...
int gpu_pm_qos_command(struct exynos_context *platform, gpu_pmqos_state state)
{
	...
	case GPU_CONTROL_PM_QOS_DEINIT:
		if (pm_qos_request_active(&exynos5_g3d_mif_min_qos))
			pm_qos_remove_request(&exynos5_g3d_mif_min_qos);
		if (platform->pmqos_mif_max_clock && pm_qos_request_active(&exynos5_g3d_mif_max_qos))
			pm_qos_remove_request(&exynos5_g3d_mif_max_qos);
		if (!platform->pmqos_int_disable && pm_qos_request_active(&exynos5_g3d_int_qos))
			pm_qos_remove_request(&exynos5_g3d_int_qos);
		if (pm_qos_request_active(&exynos5_g3d_cpu_cluster0_min_qos))
			pm_qos_remove_request(&exynos5_g3d_cpu_cluster0_min_qos);
		if (pm_qos_request_active(&exynos5_g3d_cpu_cluster1_max_qos))
			pm_qos_remove_request(&exynos5_g3d_cpu_cluster1_max_qos);
		if (platform->boost_egl_min_lock && pm_qos_request_active(&exynos5_g3d_cpu_cluster1_min_qos))
	...
	case GPU_CONTROL_PM_QOS_SET:
		...
		if (platform->perf_gathering_status) {
			...
			if (platform->pmqos_mif_max_clock &&
				...
				pm_qos_request_active(&exynos5_g3d_mif_max_qos))
		...
		if (!platform->boost_is_enabled && pm_qos_request_active(&exynos5_g3d_cpu_cluster1_max_qos))
	...
	case GPU_CONTROL_PM_QOS_EGL_SET:
		if (pm_qos_request_active(&exynos5_g3d_cpu_cluster1_min_qos))
...
int gpu_mif_pmqos(struct exynos_context *platform, int mem_freq)
{
	...
	if(!platform->devfreq_status)
		return 0;
	if(prev_freq != mem_freq && pm_qos_request_active(&exynos5_g3d_mif_min_qos))
