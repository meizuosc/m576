...
#if defined(CONFIG_MEIZU_DECON_LCD_S6E3FA3)
...
static int dsim_get_regulator(struct dsim_device *dsim)
{
	...
	g_disp_regulator.vci = devm_regulator_get(dsim->dev, "vci-lcd");
	if (IS_ERR(g_disp_regulator.vci)) {
		dsim_err("lcd vci regulator is not available.\n");
		return -EINVAL;
	}

	g_disp_regulator.vdd3 = devm_regulator_get(dsim->dev, "vdd3-lcd");
	if (IS_ERR(g_disp_regulator.vdd3)) {
		dsim_err("lcd vdd3 regulator is not available.\n");
		return -EINVAL;
	}
...
static int dsim_set_panel_power(struct dsim_device *dsim, bool on)
{
	...
	if (on) {
		...
		ret = regulator_enable(g_disp_regulator.vci);
		ret = regulator_enable(g_disp_regulator.vdd3);
#else
		if (res->lcd_power[0] > 0) {
...
#ifdef CONFIG_MEIZU_DECON_LCD_S6E3FA3
		regulator_disable(g_disp_regulator.vci);
		regulator_disable(g_disp_regulator.vdd3);
#else
		if (res->lcd_power[0] > 0) {
...
#elif defined(CONFIG_EXYNOS_DECON_LCD_S6E8AA0)
	dsim->panel_ops = &s6e8aa0_mipi_lcd_driver;
#elif defined(CONFIG_EXYNOS_DECON_LCD_S6E8FA0)
	dsim->panel_ops = &s6e8fa0x01_mipi_lcd_driver;
