	...
	u32 status)
{
	...
	/* 1. sub frame done */
	framemgr_e_barrier_irqs(sub_framemgr, FMGR_IDX_9, flags);
	...
	framemgr_x_barrier_irqr(sub_framemgr, FMGR_IDX_9, flags);
	...
	u32 status)
{
	...
	/* 1. sub frame done */
	framemgr_e_barrier_irqs(sub_framemgr, FMGR_IDX_10, flags);
	...
	framemgr_x_barrier_irqr(sub_framemgr, FMGR_IDX_10, flags);
...
static void wq_func_shot(struct work_struct *data)
{
	...
	while (work) {
		...
		framemgr_e_barrier_irqs(grp_framemgr, FMGR_IDX_11, flags);
		...
		framemgr_x_barrier_irqr(grp_framemgr, FMGR_IDX_11, flags);
#ifdef ENABLE_CLOCK_GATE
		if (fcount == 1 &&
...
static inline void print_framemgr_spinlock_usage(struct fimc_is_core *core)
{
	...
	for (i = 0; i < FIMC_IS_MAX_NODES; ++i) {
		...
		if (test_bit(FIMC_IS_ISCHAIN_OPEN, &device->state)) {
			/* 1. sensor framemgr check */
			sensor = device->sensor;
			if (sensor != NULL && test_bit(FIMC_IS_SENSOR_OPEN, &sensor->state)) {
				minfo(" sensor framemgr spinlock check", device);
				framemgr = GET_DST_FRAMEMGR(sensor->vctx);
				framemgr_lock_usage_log(framemgr);
			}

			/* 2. group leader framemgr check */
			/* 3aa */
			group = &device->group_3aa;
			if (test_bit(FIMC_IS_GROUP_OPEN, &group->state)) {
				minfo(" 3aa(%d) framemgr spinlock check", device, group->id);
				framemgr = GET_SRC_FRAMEMGR(group->leader.vctx);
				framemgr_lock_usage_log(framemgr);
				framemgr = GET_DST_FRAMEMGR(group->leader.vctx);
				framemgr_lock_usage_log(framemgr);
			}
			...
			group = &device->group_isp;
			if (test_bit(FIMC_IS_GROUP_OPEN, &group->state)) {
				minfo(" isp(%d) framemgr spinlock check", device, group->id);
				framemgr = GET_SRC_FRAMEMGR(group->leader.vctx);
				framemgr_lock_usage_log(framemgr);
			}

			/* 3. other subdev framegmgr check */
			subdev = &device->taac;
			if (test_bit(FIMC_IS_SUBDEV_OPEN, &subdev->state)) {
				minfo(" 3aac framemgr spinlock check", device);
				framemgr = GET_DST_FRAMEMGR(subdev->vctx);
				framemgr_lock_usage_log(framemgr);
			}
			subdev = &device->taap;
			if (test_bit(FIMC_IS_SUBDEV_OPEN, &subdev->state)) {
				minfo(" 3aap framemgr spinlock check", device);
				framemgr = GET_DST_FRAMEMGR(subdev->vctx);
				framemgr_lock_usage_log(framemgr);
			}
			subdev = &device->scc;
			if (test_bit(FIMC_IS_SUBDEV_OPEN, &subdev->state)) {
				minfo(" scc framemgr spinlock check", device);
				framemgr = GET_DST_FRAMEMGR(subdev->vctx);
				framemgr_lock_usage_log(framemgr);
			}
			subdev = &device->scp;
			if (test_bit(FIMC_IS_SUBDEV_OPEN, &subdev->state)) {
				minfo(" scp framemgr spinlock check", device);
				framemgr = GET_DST_FRAMEMGR(subdev->vctx);
				framemgr_lock_usage_log(framemgr);
			}
		}
	...
	for (i = 0; i < FIMC_IS_MAX_NODES; ++i) {
		...
		if (test_bit(FIMC_IS_ISCHAIN_OPEN_SENSOR, &device->state)) {
			...
			if (test_bit(FIMC_IS_GROUP_READY, &device->group_3aa.state)) {
				...
				framemgr_e_barrier_irqs(framemgr, FMGR_IDX_12, flags);
				scount_3ax = framemgr->frame_pro_cnt;
				shot_count += scount_3ax;
				framemgr_x_barrier_irqr(framemgr, FMGR_IDX_12, flags);
			}
...
int fimc_is_hw_regdump(struct fimc_is_interface *this)
{
	...
	regs = (this->regs);
	for (i = 0; i < 16; ++i)
		printk(KERN_DEBUG "MCTL Raw[%d] : %08X\n", i, readl(regs + (4 * i)));

	printk(KERN_DEBUG "\n### COMMON REGS dump ###\n");
	regs = this->com_regs;
	for (i = 0; i < 64; ++i)
		printk(KERN_DEBUG "MCTL[%d] : %08X\n", i, readl(regs + (4 * i)));
	...
	ulong end)
{
	...
	printk(KERN_ERR "Memory Dump(0x%08lX ~ 0x%08lX)\n", start, end);
	...
	while ((ulong)cur <= end) {
		...
		snprintf(term, sizeof(term), "%08lX   ", *cur);
		snprintf(&sentence[offset], sizeof(sentence) - offset, "%s", term);
		offset += strlen(term);
		cur++;
		items++;
