...
set_info:
	...
	if (framemgr->id & FRAMEMGR_ID_SHOT) {
		...
		frame->cookie_shot = (ulong)vb2_plane_cookie(vb, spare);
	...
	struct v4l2_buffer *buf)
{
	...
	framemgr_e_barrier_irq(framemgr, FMGR_IDX_1);
	qcount = framemgr->frame_req_cnt +
		framemgr->frame_pro_cnt +
		framemgr->frame_com_cnt;
	framemgr_x_barrier_irq(framemgr, FMGR_IDX_1);
	...
	enum v4l2_buf_type type)
{
	...
	framemgr_e_barrier_irq(framemgr, FMGR_IDX_2);
	qcount = framemgr->frame_req_cnt +
		framemgr->frame_pro_cnt +
		framemgr->frame_com_cnt;
	framemgr_x_barrier_irq(framemgr, FMGR_IDX_2);
