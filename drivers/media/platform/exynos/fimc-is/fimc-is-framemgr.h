...
#define framemgr_e_barrier_irqs(this, index, flag) \
	{(this->p_index) |= index;spin_lock_irqsave(&this->slock, flag);}
#define framemgr_x_barrier_irqr(this, index, flag) \
	{spin_unlock_irqrestore(&this->slock, flag);(this->p_index) &= (~index);}
#define framemgr_e_barrier_irq(this, index) \
	{(this->p_index) |= index;spin_lock_irq(&this->slock);}
#define framemgr_x_barrier_irq(this, index) \
	{spin_unlock_irq(&this->slock);(this->p_index) &= (~index);}
#define framemgr_e_barrier(this, index) \
	{(this->p_index) |= index;spin_lock(&this->slock);}
#define framemgr_x_barrier(this, index) \
	{spin_unlock(&this->slock);(this->p_index) &= (~index);}
...
struct fimc_is_framemgr {
	...
	u32			id;
	/* for checking spinlock of usages */
	ulong			p_index;
};
