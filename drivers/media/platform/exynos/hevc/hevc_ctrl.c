...
int hevc_alloc_firmware(struct hevc_dev *dev)
{
	...
	hevc_info("Port A: %08zu Port B: %08zu (FW: %08llx size: %08zu)\n",
			dev->port_a, dev->port_b,
			(unsigned long long)hevc_bitproc_phys,
			firmware_size);
...
int hevc_load_firmware(struct hevc_dev *dev)
{
	...
	hevc_debug(2, "Ret of request_firmware: %d Size: %zu\n", err, fw_blob->size);
...
static inline void hevc_init_memctrl(struct hevc_dev *dev)
{
	...
	hevc_debug(2, "Base Address : %08zu\n", dev->port_a);
}
