...
int hevc_init_pm(struct hevc_dev *dev)
{
	...
	pm->clock = clk_get(dev->device, "aclk_hevc");
#endif
	if (IS_ERR(pm->clock)) {
...
#else
	int index;
	char *str_child[15] = {"pclk_hevc", "aclk_lh_async_s_hevc_0", "aclk_lh_async_s_hevc_1",
...
#else
	parent_clk = clk_get(dev->device, "dout_aclk_hevc_532");
#endif
	if (IS_ERR(parent_clk)) {
		hevc_err("failed to get %s clock\n", __clk_get_name(parent_clk));
