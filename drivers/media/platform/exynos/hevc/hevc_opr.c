		  ...
		  unsigned int start_num_byte, unsigned int strm_size)
{
	...
	hevc_debug(2, "inst_no: %d, buf_addr: 0x%llx\n", ctx->inst_no,
		(unsigned long long)buf_addr);
	hevc_debug(2, "strm_size: 0x%08x cpb_buf_size: 0x%llx\n",
			 strm_size, (unsigned long long)cpb_buf_size);
...
int hevc_set_dec_frame_buffer(struct hevc_ctx *ctx)
{
	...
	list_for_each_entry(buf, buf_queue, list) {
		...
		for (j = 0; j < raw->num_planes; j++) {
			hevc_debug(2, "buf->planes.raw[%d] = 0x%llx\n", j,
				(unsigned long long)buf->planes.raw[j]);
	...
	hevc_debug(2, "Buf1: %zu, buf_size1: %ld (frames %d)\n",
...
static int hevc_set_dynamic_dpb(struct hevc_ctx *ctx, struct hevc_buf *dst_vb)
{
	...
	hevc_debug(2, "Dst addr [%d] = 0x%llx\n", dst_index,
