...
static void hevc_handle_frame_new(struct hevc_ctx *ctx, unsigned int err)
{
	...
	list_for_each_entry(dst_buf, dst_queue_addr, list) {
		...
		hevc_debug(2, "0x%08llx, 0x%08llx",
				(unsigned long long)hevc_mem_plane_addr(ctx,
...
static void hevc_handle_ref_frame(struct hevc_ctx *ctx)
{
	...
	if ((buf_addr == dec_addr) && (dec_buf->used == 1)) {
		hevc_debug(2, "Find dec buffer y = 0x%llx\n",
		...
		hevc_debug(2, "Can't find buffer for addr = 0x%llx\n",
			(unsigned long long)dec_addr);
		hevc_debug(2, "Expected addr = 0x%llx, used = %d\n",
