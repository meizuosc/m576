				...
				struct fimg2d_dma_group *dma)
{
	...
	wr(FIMG2D_ADDR(dma->base.iova), FIMG2D_SRC_BASE_ADDR_REG);
	...
	if (s->order < ARGB_ORDER_END) {	/* argb */
		...
		wr(FIMG2D_ADDR(dma->plane2.iova), FIMG2D_SRC_PLANE2_BASE_ADDR_REG);
	}
				...
				struct fimg2d_dma_group *dma)
{
	...
	if (d->order < ARGB_ORDER_END) {
		...
		wr(FIMG2D_ADDR(dma->plane2.iova), FIMG2D_DST_PLANE2_BASE_ADDR_REG);
	}
				...
				struct fimg2d_dma_group *dma)
{
	...
	wr(FIMG2D_ADDR(dma->base.iova), FIMG2D_MSK_BASE_ADDR_REG);
