...
struct fimc_is_flite_frame_list {
	struct fimc_is_flite_frame		work[NUM_STAT_INTERNAL_BUF];
	/*spin lock*/
	spinlock_t			slock_free;
	spinlock_t			slock_request;
	spinlock_t			slock_complete;
	/*head list*/
	struct list_head		work_free_head;
	u32				work_free_cnt;
	struct list_head		work_request_head;
	u32				work_request_cnt;
	struct list_head		work_complete_head;
	u32				work_complete_cnt;
	/*wait que*/
...
#if defined(CONFIG_CAMERA_SUPPORT_PDAF)
	struct fimc_is_flite_frame_list	frame_list;
#endif
...
#if defined(CONFIG_CAMERA_SUPPORT_PDAF)
struct fimc_is_flite_statdata {
	int index;
	char pdaf_stat[PDAF_STAT_SIZE];
};
#endif
