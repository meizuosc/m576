...
void	download( OIS_UWORD u16_type, OIS_UWORD u16_coef_type ){
	...
	module_id = mz_ois_get_module_id();
	if (REAR_SHARP_MODULE == module_id) {
		pr_info("%s(), use sharp module specific\n", __func__);
		DOWNLOAD_COEF_LEN = DOWNLOAD_COEF_LEN_SHARP;
		DOWNLOAD_COEF = DOWNLOAD_COEF_SHARP;
		DOWNLOAD_BIN_LEN = DOWNLOAD_BIN_LEN_SHARP;
		DOWNLOAD_BIN = DOWNLOAD_BIN_SHARP;
	} else if (REAR_PRIMAX_MODULE == module_id) {
		pr_info("%s(), use primax module specific\n", __func__);
		DOWNLOAD_COEF_LEN = DOWNLOAD_COEF_LEN_PRIMAX;
		DOWNLOAD_COEF = DOWNLOAD_COEF_PRIMAX;
		DOWNLOAD_BIN_LEN = DOWNLOAD_BIN_LEN_PRIMAX;
		DOWNLOAD_BIN = DOWNLOAD_BIN_PRIMAX;
	} else {
		pr_err("%s(), invalid module_id:%d, return directly!\n",
			__func__, module_id);
		return;
	}
...
//  **** Scence parameter
//  *****************************************************
// ==> ROHM 2015/04/02	Modified
#define	ANGLE_LIMIT	(OIS_UWORD)(0x2BC0 * 110 / 100)		// GYRSNS * limit[deg]
#define	G_SENSE		(875/10)							// [LSB/dps]
// <== ROHM 2015/04/02	Modified
...
// ==> RHM_HT 2013/11/25	Modified
	...
	OIS_UWORD u16_dat_SCENE_SPORT_3[3] = { 0x7F80,   ANGLE_LIMIT,   G_SENSE * 60,    };	// 
...
/*
* This function is add by qudao of MEIZU
*/
ADJ_STS mz_func_adj_angle_limit(OIS_UBYTE degree)
{
	OIS_UWORD u16_dat;
	OIS_ULONG u32_dat;
	OIS_UWORD MZ_ANGLE_LIMIT = (OIS_UWORD)(0x2BC0 * degree / 100);

	pr_info("%s(), degree:%d, MZ_ANGLE_LIMIT:%d\n",
		__func__, degree, MZ_ANGLE_LIMIT);

	// Set parameter to the OIS controller
	I2C_OIS_mem_write(_M_X_LMT, MZ_ANGLE_LIMIT);
	I2C_OIS_mem_write(_M_Y_LMT, MZ_ANGLE_LIMIT);

	// ==> 2015/07/13 RHM_HT		Modified
	u32_dat = ( INTG__INPUT * 16384L ) / MZ_ANGLE_LIMIT;		// X2 * 4000h / X1

	if ( u32_dat >= 0x7FFF) {
		u16_dat = 0x7FFF;
	} else {
		u16_dat = (OIS_UWORD)u32_dat;
	}
	// <== 2015/07/13 RHM_HT		Modified
	...
	u16_dat = ( KGNTG_VALUE * MZ_ANGLE_LIMIT ) / 16384L;	// X3 * X1 / 4000h
...
ADJ_STS	func_SET_SCENE_PARAM_for_NewGYRO_Fil(OIS_UBYTE u16_scene, OIS_UBYTE u16_mode, OIS_UBYTE filter, OIS_UBYTE range, const _FACT_ADJ *param)	// RHM_HT 2013/04/15	Change "typedef" of return value
{
	...
	OIS_UWORD u16_dat_SCENE_SPORT_1[4] = { 0x7FE0,   ANGLE_LIMIT,   G_SENSE * 60,   0x0300,  };	// 60dps
// szx_2014/12/24 ===>
	OIS_UWORD u16_dat_SCENE_SPORT_2[4] = { 0x7F80,   ANGLE_LIMIT,   G_SENSE * 60,   0x0080,  };	// 60dps
	OIS_UWORD u16_dat_SCENE_SPORT_3[4] = { 0x7FF0,   ANGLE_LIMIT,   G_SENSE * 60,   0x0300,  };	// 60dps
// szx_2014/12/24 <===
