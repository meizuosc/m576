...
void tasklet_flite_str0(unsigned long data)
{
	...
	if (unlikely(list_empty(&group_3aa->smp_trigger.wait_list))) {
		...
		if (((g_print_cnt % LOG_INTERVAL_OF_DROPS) == 0) ||
			...
			info("in str0, grp1(res %d, rcnt %d, scnt %d), "
...
void tasklet_flite_str2(unsigned long data)
{
        ...
        if (unlikely(list_empty(&group_3aa->smp_trigger.wait_list))) {
                ...
                if (((g_print_cnt % LOG_INTERVAL_OF_DROPS) == 0) ||
                        (g_print_cnt < LOG_INTERVAL_OF_DROPS)) {
                        info("in str2, grp1(res %d, rcnt %d, scnt %d), "
                                "grp2(res %d, rcnt %d, scnt %d), "
                                "fcount %d(%d, %d) pcount %d\n",
                                groupmgr->group_smp_res[group_3aa->slot].count,
                                atomic_read(&group_3aa->rcount),
...
static int set_req_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame *work)
{
	int ret = 0;
	unsigned long flags;
...
static int get_req_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame **work)
{
	int ret = 0;
	unsigned long flags;
	...
	if (work) {
		...
		if (this->work_request_cnt) {
			*work = container_of(this->work_request_head.next,
					struct fimc_is_flite_frame, list);
			list_del(&(*work)->list);
			this->work_request_cnt--;
		} else
			*work = NULL;
...
static int set_free_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame *work)
{
	int ret = 0;
	unsigned long flags;
...
static int get_free_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame **work)
{
	int ret = 0;
	unsigned long flags;
...
static int set_com_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame *work)
{
	int ret = 0;
	unsigned long flags;
	...
	if (work) {
		spin_lock_irqsave(&this->slock_complete, flags);
		list_add_tail(&work->list, &this->work_complete_head);
		this->work_complete_cnt++;
		spin_unlock_irqrestore(&this->slock_complete, flags);
	} else {
		ret = -EFAULT;
		err("item is null ptr\n");
	}
...
static int get_com_work(struct fimc_is_flite_frame_list *this,
	struct fimc_is_flite_frame **work)
{
	int ret = 0;
	unsigned long flags;
	...
	if (work) {
		spin_lock_irqsave(&this->slock_complete, flags);
		if (this->work_complete_cnt) {
			*work = container_of(this->work_complete_head.next,
					struct fimc_is_flite_frame, list);
			list_del(&(*work)->list);
			this->work_complete_cnt--;
		} else
...
int fimc_is_statdata_copy_to_user(struct v4l2_subdev *subdev, void __user *p)
{
	int ret = 0;
    struct fimc_is_device_flite *flite;
	struct fimc_is_flite_frame *work;
	struct fimc_is_flite_statdata *statdata;
	...
	statdata = kzalloc(sizeof(struct fimc_is_flite_statdata), GFP_KERNEL);
	if (!statdata) {
		err("alloc statdata is NULL");
		ret = -ENOMEM;
		goto p_err;
	}

	if (!test_bit(FLITE_START_STREAM, &flite->state)) {
		mwarn("can't get stat data as stream off", flite);
		ret = -EINVAL;
		goto p_err;
	}
	
	/* To get StatData, user can use ioctl. But unlike othere devices, sensor is used to make outdata with variable frame.
	So polling time in hal is shorter than framerate, empty buffer can be happen.
	Prevent this, wait event is used. And this is needed to be verified with high fps. Kundong.Kim@samsung.com*/

	ret = wait_event_interruptible(flite->frame_list.done_wq,flite->frame_list.work_complete_cnt);
	get_com_work(&flite->frame_list,&work);
	if(work) {
		statdata->index = work->id;
		memcpy(statdata->pdaf_stat,(char *)work->kvaddr_shot,sizeof(statdata->pdaf_stat));
		ret = copy_to_user((void __user *)p, statdata, sizeof(struct fimc_is_flite_statdata));
		set_free_work(&flite->frame_list,work);
	}
p_err:
	kfree(statdata);
	return ret;
}
void tasklet_flite_str_stat(unsigned long data)
{
    struct v4l2_subdev *subdev;
    struct fimc_is_device_flite *flite;
	struct fimc_is_flite_frame *work;
	...
	/* At this moment, set buffer for FIMCBNS is best at there. Kundong.Kim@samsung.com*/
	
	get_free_work(&flite->frame_list,&work);
	if(work) {
		flite_s_buffer_addr(flite, FLITE_A_SLOT_VALID, work->dvaddr_shot);
		flite_hw_set_output_dma(flite->base_reg, true);
		set_req_work(&flite->frame_list,work);
	} else
		flite_hw_set_output_dma(flite->base_reg, false);

}

void tasklet_flite_end_stat(unsigned long data)
{
    struct fimc_is_device_flite *flite;
	struct fimc_is_flite_frame *work;
	...
	get_req_work(&flite->frame_list,&work);
	if(work) {
		set_com_work(&flite->frame_list,work);
	} else
		flite_hw_set_output_dma(flite->base_reg, false);

	/* This should be fixed. How can handle wq in case of empty of work que. Kundong.Kim@samsung.com*/
	wake_up(&flite->frame_list.done_wq);

}

int fimc_is_stat_flite_open(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_device_flite *flite;
...
int fimc_is_stat_flite_close(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_device_flite *flite;
...
static int stat_flite_init(struct v4l2_subdev *subdev, u32 value)
{
	int ret = 0;
	int i = 0;
	struct fimc_is_device_flite *flite;
	...
	flite->csi = value;
	spin_lock_init(&flite->frame_list.slock_complete);
	spin_lock_init(&flite->frame_list.slock_free);
	spin_lock_init(&flite->frame_list.slock_request);

	init_waitqueue_head(&flite->frame_list.done_wq);


	/* Once frame list is not used for getting stat data, order of stat frame can be changed.
	To prevent this, 3 state lists are used. Kundong.Kim@samsung.com*/

	flite->frame_list.work_free_cnt = 0;
	flite->frame_list.work_request_cnt	= 0;
	flite->frame_list.work_complete_cnt	= 0;
	INIT_LIST_HEAD(&flite->frame_list.work_free_head);
	INIT_LIST_HEAD(&flite->frame_list.work_request_head);
	INIT_LIST_HEAD(&flite->frame_list.work_complete_head);

	for (i = 0; i < NUM_STAT_INTERNAL_BUF; i++) {
		flite->frame_list.work[i].id = i;
		set_free_work(&flite->frame_list, &flite->frame_list.work[i]);
	}
...
static int stat_flite_stream_on(struct v4l2_subdev *subdev,
	struct fimc_is_device_flite *flite)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < NUM_STAT_INTERNAL_BUF; i++) {
		flite->frame_list.work[i].dvaddr_shot = device->ischain->imemory.dvaddr_stat[i];
		flite->frame_list.work[i].kvaddr_shot = device->ischain->imemory.kvaddr_stat[i];
	}
	...
	if(work) {
		set_req_work(&flite->frame_list,work);
		//flite_s_buffer_addr(flite, FLITE_A_SLOT_VALID, flite->dvaddr_shot);
		flite_s_buffer_addr(flite, FLITE_A_SLOT_VALID, work->dvaddr_shot);
	}

	flite_hw_set_output_dma(flite->base_reg, true);
	tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str_stat, (unsigned long)subdev);
	tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end_stat, (unsigned long)subdev);
	flite_hw_set_output_local(flite->base_reg, false);

	/* This is Hack code for FimcBNS DMA. In future, this will be modified by right solution . Kundong.Kim@samsung.com*/
	image->format.pixelformat = V4L2_PIX_FMT_SBGGR16;
	image->window.o_width = PDAF_STAT_SIZE;
	image->window.o_height = 1;
	image->window.width = PDAF_STAT_SIZE;
	image->window.height= 1;
	image->window.offs_h= 0;
	image->window.offs_v= 0;
	image->window.otf_width= PDAF_STAT_SIZE;
	image->window.otf_height= 1;

	flite_hw_set_dma_fmt(flite->base_reg, image->format.pixelformat);
	flite_hw_set_cam_channel(flite->base_reg, otf_setting);
	flite_hw_set_cam_source_size(flite->base_reg, image);
	flite_hw_set_dma_offset(flite->base_reg, image);
	flite_hw_set_camera_type(flite->base_reg);

	/* This is Hard code for PDAF STAT DT, This will be modified by right solution.dong.Kim@samsung.com*/
 	cfg = readl(flite->base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
	cfg |= (HW_FORMAT_PDAF_STAT << 24);
	writel(cfg, flite->base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));

	flite_hw_set_inverse_polarity(flite->base_reg);
	flite_hw_set_interrupt_source(flite->base_reg);
	flite_hw_set_window_offset(flite->base_reg, image);

	flite_hw_set_mux(flite->base_reg, flite->csi, flite->instance);
#ifdef CONFIG_SOC_EXYNOS7420
		if (device->ischain && !test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &device->ischain->state)) {
			/* set mux setting for otf between 3aa and isp */
			flite_hw_set_mux_taa_isp(flite->base_reg,
				device->ischain->group_3aa.id - GROUP_ID_3AA0,
				device->ischain->group_isp.id - GROUP_ID_ISP0);
		}
#endif

	flite_hw_set_last_capture_end_clear(flite->base_reg);
	flite_hw_set_capture_start(flite->base_reg);
	set_bit(FLITE_START_STREAM, &flite->state);
...
static int stat_flite_stream_off(struct v4l2_subdev *subdev,
	struct fimc_is_device_flite *flite,
	bool nowait)
{
	int ret = 0;
	...
	//clean requsted reasource before stream off
	for (i = 0; i < NUM_STAT_INTERNAL_BUF; i++) {
		get_req_work(&flite->frame_list,&work);
		if(work)
			set_com_work(&flite->frame_list,work);
	...
	wake_up_all(&flite->frame_list.done_wq);
 ...
 * @0X : waiting flag
 */
static int stat_flite_s_stream(struct v4l2_subdev *subdev, int enable)
{
	int ret = 0;
	bool nowait;
	struct fimc_is_device_flite *flite;
	...
	if (enable) {
		ret = stat_flite_stream_on(subdev, flite);
		if (ret) {
			err("stat_flite_stream_on is fail(%d)", ret);
			goto p_err;
		}
	} else {
		ret = stat_flite_stream_off(subdev, flite, nowait);
		if (ret) {
			err("stat_flite_stream_off is fail(%d)", ret);
			goto p_err;
		}
	}
...
static int stat_flite_s_format(struct v4l2_subdev *subdev, struct v4l2_mbus_framefmt *fmt)
{
	int ret = 0;
	struct fimc_is_device_flite *flite;
...
static int stat_flite_s_buffer(struct v4l2_subdev *subdev, void *buf, unsigned int *size)
{
    int ret = 0;
    struct fimc_is_device_flite *flite;
    struct fimc_is_framemgr *framemgr;
...
static int stat_flite_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	struct fimc_is_device_flite *flite;
...
static const struct v4l2_subdev_core_ops stat_core_ops = {
	.init = stat_flite_init,
	.s_ctrl = stat_flite_s_ctrl,
};

static const struct v4l2_subdev_video_ops stat_video_ops = {
	.s_stream = stat_flite_s_stream,
	.s_mbus_fmt = stat_flite_s_format,
	.s_rx_buffer= stat_flite_s_buffer
};

static const struct v4l2_subdev_ops stat_subdev_ops = {
	.core = &stat_core_ops,
	.video = &stat_video_ops
};

int fimc_is_statflite_probe(struct fimc_is_device_sensor *device,
	u32 instance)
{
	int ret = 0;
	...
	subdev_statflite = kzalloc(sizeof(struct v4l2_subdev), GFP_KERNEL);
	if (!subdev_statflite) {
		merr("subdev_statflite is NULL", device);
		ret = -ENOMEM;
		goto err_alloc_subdev_flite;
	}
	device->subdev_statflite= subdev_statflite;
	...
	/* pointer to me from device sensor */
	flite->subdev = &device->subdev_statflite;
	flite->instance = instance;
	init_waitqueue_head(&flite->wait_queue);
	switch(instance) {
	case FLITE_ID_A:
	...
	v4l2_subdev_init(subdev_statflite, &stat_subdev_ops);
	v4l2_set_subdevdata(subdev_statflite, (void *)flite);
	v4l2_set_subdev_hostdata(subdev_statflite, device);
	snprintf(subdev_statflite->name, V4L2_SUBDEV_NAME_SIZE, "statflite-subdev.%d", instance);
	ret = v4l2_device_register_subdev(&device->v4l2_dev, subdev_statflite);
	if (ret) {
		merr("v4l2_device_register_subdev is fail(%d)", device, ret);
		goto err_reg_v4l2_subdev;
	}
