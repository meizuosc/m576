			  ...
			  const struct arizona_jd_state *new_state)
{
	...
	dev_info(info->arizona->dev,"%s:old_state=%p,new_state=%p\n",__func__,info->state,new_state);
				...
				int val)
{
	...
	if (val & MICD_LVL_0_TO_7) {
		...
		dev_info(arizona->dev, "Mic button detected: lvl=%x, ffs(lvl)-1=%d\n",lvl,ffs(lvl) - 1);
		...
		if (lvl && ffs(lvl) - 1 < info->num_micd_ranges) {
			...
			dev_info(arizona->dev, "Mic button detected: key=%d\n",key);
...
static irqreturn_t arizona_hpdet_handler(int irq, void *data)
{
	...
	if (arizona->hp_impedance > 33 && arizona->hp_impedance < 10000) {
		dev_info(arizona->dev, "%s:%d,prev_hp_impedance=%d\n",__func__,__LINE__,prev_hp_impedance);
		if(abs(prev_hp_impedance - arizona->hp_impedance) > 1) {
			printk("%s:HP_IMPEDANCE: abs(current - previous)  > 1 ohm %d\n",__func__,__LINE__);
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	if (info->last_jackdet == present) {
...
#if 1
		/*make hifi/8998 and mute high to detect headset for wm8998*/
		es9018k2m_blocking_notifier_call_chain(0);
		msleep(100);
#endif
		...
		es9018k2m_blocking_notifier_call_chain(1);
#endif
