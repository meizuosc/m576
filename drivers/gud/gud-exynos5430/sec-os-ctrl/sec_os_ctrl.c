		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned int big_cpu_num = 0;
	unsigned int little_cpu_num = 0;
	...
	/* Find CPU number of each cluster */
	big_cpu_num = cpumask_weight(&hmp_fast_cpu_mask);
	if (big_cpu_num == 0) {
		pr_err("[big] big core isn't present\n");
		return count;
	}

	little_cpu_num = cpumask_weight(&hmp_slow_cpu_mask);
	if (little_cpu_num == 0) {
		pr_err("[LITTLE] LITTLE core isn't present\n");
		return count;
	}
	...
	if (buf[0] == 'L') {
		...
		} else if ((core_num >= 0) && (core_num < little_cpu_num)) {	/* From core 0 to core 3 */
			new_core = core_num;
		} else {
		...
		} else if ((core_num >= 0) && (core_num < big_cpu_num)) {	/* From core 0 to core 3 */
			new_core = core_num + 4;
		} else {
