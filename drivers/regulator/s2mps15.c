				     ...
				     unsigned int mode)
{
	...
	/* leave LDO2 state for SIM issue */
	if (id == S2MPS15_LDO2)
		goto skip_enable;
...
static int s2mps15_pmic_probe(struct platform_device *pdev)
{
	...
	/* RTC Low jitter mode and BT&CP 32K on */
	ret = sec_reg_update(iodev, S2MPS15_REG_RTC_BUF, 0x16, 0x16);
	if (ret) {
	...
	/* 15s Mannual reset */
	ret = sec_reg_write(iodev, S2MPS15_REG_CTRL1, 0x1A);
	if (ret) {
		dev_err(&pdev->dev, "fail set mannual reset\n");
		goto err;
	}

	/* 7s Warm reset */
	ret = sec_reg_write(iodev, S2MPS15_REG_CTRL3, 0x14);
	if (ret) {
		dev_err(&pdev->dev, "fail set warm reset\n");
		goto err;
	}

	sec_reg_read(iodev, S2MPS15_REG_ST1, &temp);
	pr_info("%s: STATUS1: 0x%x\n", __func__, temp);
	sec_reg_read(iodev, S2MPS15_REG_ST2, &temp);
	pr_info("%s: STATUS2: 0x%x\n", __func__, temp);
	sec_reg_read(iodev, S2MPS15_REG_PWRONSRC, &temp);
	pr_info("%s: PWRONSRC: 0x%x\n", __func__, temp);
	sec_reg_read(iodev, S2MPS15_REG_OFFSRC, &temp);
