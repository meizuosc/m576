...
#define INTIAL_CODE_FILE_PATH_NAME "data/tp.txt"
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	printk("hwj data->cfg_name = %s dev->init_name = %s\n", data->cfg_name, dev->init_name);
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	fp = filp_open(INTIAL_CODE_FILE_PATH_NAME, O_RDWR|O_CREAT, 777);
	if (IS_ERR(fp))
	{
		printk("%s create file error", __FUNCTION__);
		return 0;
	}

	error = vfs_stat(INTIAL_CODE_FILE_PATH_NAME, &str);
	printk("error =%d, str.size = %d\n", error, (int)str.size);
	if(str.size == 0)
	{
		return -1;
	}
	...
	cfg->size = 2705;
	cfg->data= kzalloc(2705, GFP_KERNEL);
	vfs_read(fp, (char *)cfg->data, str.size, &pos);
	printk("hwj cfg->size = %d, cfg->data = %s\n", (int)cfg->size, cfg->data);
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	printk("hwj mxt_regulator_enable\n");
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	struct device *dev = &data->client->dev;
	int error;
	printk("hwj mxt_probe_regulators\n");
#if 1
				   ...
				   const struct i2c_device_id *id)
{
	...
	if (!data->pdata && client->dev.of_node)
		...
		if (data->pdata)
			printk("hwj atmel_mxt540_parse_dt success\n");
