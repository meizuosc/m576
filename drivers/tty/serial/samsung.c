...
static bool s3c24xx_serial_lpm_suspend(struct s3c24xx_uart_port *ourport)
{
	if ((atomic_read(&ourport->serial_suspend)
		|| ourport->port.suspended) && ourport->domain == DOMAIN_AUD)
		return 1;
...
rx_use_cpu:
	...
	while (max_count-- > 0) {
		...
		if (BT_UART_NUMBER == ourport->pdev->id &&
			rx_cnt_debug < 64) {
			printk("rx[%d] = 0x%x, uerstat:0x%x, flag:0x%x\n",
...
{
	if (ourport->domain == DOMAIN_AUD)
		lpass_get_sync(&ourport->pdev->dev);
}

static void aud_uart_put_sync(struct s3c24xx_uart_port *ourport)
{
	if (ourport->domain == DOMAIN_AUD)
		lpass_put_sync(&ourport->pdev->dev);
}
...
static int s3c24xx_serial_setclk(struct s3c24xx_uart_port *ourport, int rate)
{
	struct clk *clk;
...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	if (ourport->domain == DOMAIN_AUD) {
		...
		// set aud_uart sclk
		ret = s3c24xx_serial_setclk(ourport, 196608032);
		if (ret < 0)
			pr_info("%s:fail to set aud_sclk\n", __func__);
