...
static int s2m_rtc_stop_alarm(struct s2m_rtc_info *info)
{
	...
	dev_info(info->dev, "%s: stop alarm\n", __func__);

	mutex_lock(&info->lock);

	for (i = 0; i < NR_RTC_CNT_REGS; i++) {
		ret = sec_rtc_update(info->iodev, S2M_ALARM0_SEC + i, 0, ALARM_ENABLE_MASK);
		if (ret < 0) {
			dev_err(info->dev, "%s: stop alarm failed!!!!!\n", __func__);
			goto out;
		}
	}
...
#ifdef CONFIG_RECOVERY_KERNEL
	printk("%s:irq(%d), charge_mode:%d\n", __func__, irq, charge_mode);
#else
	printk("%s:irq(%d)\n", __func__, irq);
#endif
...
static void s2m_rtc_shutdown(struct platform_device *pdev)
{
	...
	/* disable alarm0, otherwise alarm0 event will occur in charge mode,
	 * then it can't poweroff by remove charger.
	 */
