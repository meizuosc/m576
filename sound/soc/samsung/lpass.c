...
static struct lpass_info {
	...
	struct pm_qos_request	aud_cluster0_min_num_qos;
#endif
	int			cpu_qos;
	int			kfc_qos;
	int			mif_qos;
...
void lpass_get_cpu_hotplug(void)
{
	pr_debug("%s ++\n", __func__);
#if defined(CONFIG_EXYNOS_MARCH_DYNAMIC_CPU_HOTPLUG)
	pm_qos_update_request(&lpass.aud_cluster0_min_num_qos, 3);
#else
...
void lpass_put_cpu_hotplug(void)
{
	pr_debug("%s --\n", __func__);
#if defined(CONFIG_EXYNOS_MARCH_DYNAMIC_CPU_HOTPLUG)
	pm_qos_update_request(&lpass.aud_cluster0_min_num_qos, 0);
#else
...
#if defined(CONFIG_EXYNOS_MARCH_DYNAMIC_CPU_HOTPLUG)
	pm_qos_add_request(&lpass.aud_cluster0_min_num_qos, PM_QOS_CLUSTER0_NUM_MIN, 0);
#endif
#endif
	return 0;
}
