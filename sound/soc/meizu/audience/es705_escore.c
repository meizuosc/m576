...
// NOTE: that's a workaround to fix mute issue for call
//  we run a delay work to check whether route status is active or not,
//  if not, set the route again.
//#define ES705_REROUTE
...
static void es705_reroute(struct work_struct *w)
{
    ...
    if (value) {
        dev_err(es705->dev, "++%s(): route_status : 0x%04x, re-route to %ld\n",
			    __func__, value, es705->internal_route_num);
        rc = escore_write_block(es705, es705_route_config[es705->internal_route_num].route);
    }
...
static void es705_switch_route(long route_index)
{
	...
	escore_write_block(es705,
		es705_route_config[es705->internal_route_num].route);

#if defined(ES705_REROUTE)
	// only try to reroute for voice-call & voip
	if (route_index >= ROUTE_VOICE_CALL_CT && route_index <= ROUTE_VOICE_CALL_WB_BT_NREC_OFF) {
		dev_dbg(es705->dev, "%s(): scheduling re-route work\n", __func__);
		schedule_delayed_work(&es705->reroute_work, msecs_to_jiffies(500));
	}
#endif
					...
					const char *buf, size_t count)
{
	...
	if (rc) {
		...
		msleep(50);
		escore_priv.escore_power_state = ES705_SET_POWER_STATE_VS_OVERLAY;
...
static int es705_put_reconfigure_internal_route(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct escore_priv *es705 = &escore_priv;
	if (ucontrol->value.integer.value[0] == 1) {
		dev_info(es705->dev, "%s(): reconfigure internal route: %ld\n",
			__func__, es705->internal_route_num);
		escore_write_block(es705, es705_route_config[es705->internal_route_num].route);
	}
	return 0;
}

static int es705_get_reconfigure_internal_route(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	SOC_SINGLE_EXT("CVQ Sleep",
		       ...
		       escore_get_vs_sleep,
		       escore_put_vs_sleep),
	SOC_SINGLE_EXT("Reconfigure Internal Route",
		       SND_SOC_NOPM, 0, 2, 0,
		       es705_get_reconfigure_internal_route,
		       es705_put_reconfigure_internal_route),
};
...
irqreturn_t es705_irq_work(int irq, void *data)
{
	...
	if (event_type != ES_NO_EVENT) {
		pr_debug("%s(): Notify subscribers about 0x%04x event\n",
				__func__, event_type);
		if (event_type == 0x0008) { // Route recovery mechanism for mute issue.
			// Ignore Stop route condition for DHWPT usecase
			if (escore->internal_route_num != 0 && escore->internal_route_num != 1) {
				escore_write_block(escore, es705_route_config[escore->internal_route_num].route);
				pr_warn("%s(): Waining>>>Mute recovery, re-issued internal route %ld\n",
...
#if defined(ES705_REROUTE)
    INIT_DELAYED_WORK(&escore_priv.reroute_work, es705_reroute);
#endif
