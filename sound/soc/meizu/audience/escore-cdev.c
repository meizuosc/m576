				...
				size_t count, loff_t *f_pos)
{
	...
	while (used < count) {
		...
		*end = last;
		...
		pr_debug("used = %lu parse_have = %i\n", used, parse_have);//CONFIG_LINF
		if (parse_have > 0)
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	pr_debug("firmware write count=%ld bytes\n", count);

	wr = bs = 0;
	while (wr < count) {
		bs = min(count - wr, FW_BUF_SIZE);
		BUG_ON(bs == 0);
		pr_debug("wr=%ld bs=%ld buf+wr=%p\n", wr, bs, buf + wr);
				...
				size_t count, loff_t *f_pos)
{
	...
	BUG_ON(!escore);
	dev_dbg(escore->dev, "%s() entry: count: %ld\n", __func__, count);
